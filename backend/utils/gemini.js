...
async function parseCV(text) {
  const prompt = `You are an AI assistant tasked with analyzing a CV. Extract the following information and return it as a valid JSON object with these exact keys: 'name', 'email', 'phone', 'skills', 'education', 'experience', 'work_experience', 'certifications', 'projects', 'field_of_study', 'industry', 'experience_details'. 'experience_details' should be a list of objects with 'company', 'role', and 'duration' keys (e.g., {'company': 'Upwork', 'role': 'Freelancer', 'duration': 'July 2024 – Present'}), or an empty list if not available. 'education' should be an object with 'institution', 'degree', and 'gpa' if available, or empty strings if not. 'experience' should be the total years of experience as a decimal, calculated from 'experience_details' durations (e.g., 'July 2024 – Present' to ~0.9 years as of June 19, 2025, 'Jan 2025 – Ongoing' to ~0.5 years), defaulting to 0 if unclear. 'work_experience' should be a list of strings summarizing each role (e.g., ['Upwork - Freelancer (July 2024 – Present)']), not objects. 'skills' should include all technologies and tools mentioned. If a field cannot be determined, use defaults (e.g., 'Unknown' for name, [] for lists, '' for strings, 0 for experience). Do not include additional text or Markdown outside the JSON object.\n\nCV Text:\n${text}`;
